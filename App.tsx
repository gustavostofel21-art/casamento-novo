  ...
  const fetchUserProfile = async (userId: string, retries = 3) => {
    ...
    if (currentData) {
      ...
      if (!hasRedirected.current) {
        const tabs: ViewState[] = ['dashboard', 'gastos', 'fornecedores', 'convidados', 'roteiro', 'musicas', 'galeria', 'transporte', 'presentes'];
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#e8f5e9]">
      ...
      <div className={`fixed inset-y-0 right-0 w-[80%] max-w-xs bg-olive-600 z-50 transform transition-transform duration-300 md:hidden shadow-2xl ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="py-4">
          ...
          <NavItem id="transporte" icon={Bus} label="Transporte" mobile />
          <NavItem id="presentes" icon={Gift} label="Presentes" mobile />
      ...
      <aside className="w-64 bg-olive-600 border-r border-olive-500 hidden md:flex flex-col fixed h-full z-20 shadow-[4px_0_24px_rgba(0,0,0,0.1)]">
        <div className="p-8">
          ...
          <nav className="space-y-2">
            ...
            <NavItem id="transporte" icon={Bus} label="Transporte" />
            <NavItem id="presentes" icon={Gift} label="Presentes" />
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-10 overflow-y-auto mb-20 md:mb-0">
        ...
        {loading ? (
          ...
          <>
            ...
            {view === 'transporte' && hasPermission('transporte') && <Transporte />}
            {view === 'presentes' && hasPermission('presentes') && <PresentesAdmin />}
